#!/usr/bin/env bash
# switch-bug.sh <task-id> <one-line summary...>
...
if printf '%s\n' $ROLES | grep -qx coordinator; then
  ...
  echo "switched to $bug — peers reset, coordinator self-reset scheduled"
else
  echo "switched to $bug — peers reset"
fi
